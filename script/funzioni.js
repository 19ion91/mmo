...
function abilita3(){ //eslint-disable-line
  ...
  for (let p of $("img[id^="+"part"+"]"))
    p.style.opacity = "0";
  document.getElementById("fiamma").style.opacity = "0";
  document.getElementById("trasform").style.opacity = "1";
...
function auraS(){ //eslint-disable-line
...
function invisibile(){ //eslint-disable-line
...
function reset(){
  ...
  $("#apriConf").slideUp();
  document.getElementById("fiamma").style.opacity = "0";
  document.getElementById("trasform").style.opacity = "0";
  document.getElementById("center").style.opacity="1";
...
$(document).ready(function () {
  ...
  $(document).keypress(function(e) {
    if(e.key == "a") {
      var difesa = parseInt($("#difesaTotale")[0].innerHTML);
      var danno = 2000 / (3 * difesa);
      vita = (vita - danno > 0 ? vita - danno : 0);
      $("#progress-vita").width(vita + "%");
    } else if (e.key == "f") {
      vita = 100;
      $("#progress-vita").width(vita + "%");
    }
...
function updateTotali() {
  for (let t of $(".totale")) {
    ...
    if (t.id == "attaccoTotale" && $("#fiamma")[0].style.opacity == "1") {
      tot += 200;
    }
    t.innerHTML = tot;
  }
...
function saveConf() { //eslint-disable-line
